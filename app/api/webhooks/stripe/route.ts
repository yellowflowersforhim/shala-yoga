  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        // GAP G-01 FIX: Atomic capacity check — re-verify capacity inside transaction
        ...
        // GAP G-03 FIX: Atomic conditional coupon increment to prevent over-claiming
        if (order.couponId) {
    ...
    // GAP G-02 FIX: Store processed Stripe event ID for idempotency
